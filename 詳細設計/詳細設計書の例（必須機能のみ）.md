...
<div class="mermaid">
flowchart TD
    A[Start Transaction: placeOrder()] --> B[在庫確認]
    B --> C{在庫はあるか？}
    C -- いいえ --> Z[例外発生 → Rollback]
    C -- はい --> D[在庫減算（ProductRepository）]
    D --> E[注文オブジェクトの作成（Order）]
    E --> F[注文詳細リストの作成（OrderDetail）]
    F --> G[注文保存（orderRepository.saveOrder）]
    G --> H[注文詳細保存（orderRepository.saveOrderDetails）]
    H --> I[正常終了 → Commit]
    Z --> J[トランザクション終了]
    I --> J
