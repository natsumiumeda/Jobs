...
>>>>>>> ae731e4378092a1b068a88fae7d310994f343d1e
---
...
| カート編集（F103-02） | `PUT`    | `/api/cart/items/{itemId}`        |
| カート削除(F103-03)         | `DELETE` | `/api/cart/items/{itemId}`        |
| 注文情報入力（F104）   | `POST`   | `/api/orders/confirm`       |
| 注文確定(F105)          | `POST`   | `/api/orders/`      |
...
以下は注文処理におけるトランザクションの流れを示すものです。処理全体が1つのトランザクションとして扱われ、
途中でエラーが発生した場合にはすべての処理がロールバックされます。

## フローの説明

1. **トランザクション開始**: 注文処理のメソッドが呼び出されるとトランザクションが開始されます。
2. **在庫確認**: 注文された商品の在庫を確認します。
3. **在庫ありか判定**: 在庫がある場合のみ次へ進みます。ない場合は例外を投げてロールバックされます。
4. **在庫減算**: 在庫数を減らします。
5. **注文情報作成**: 注文のメイン情報を作成します（注文者、日付など）。
6. **注文詳細作成**: 各商品ごとの注文明細を作成します。
7. **注文情報保存**: 注文情報をデータベースに保存します。
8. **注文詳細保存**: 注文明細もデータベースに保存します。
9. **トランザクションコミット**: すべての処理が正常に完了した場合、トランザクションが確定されます。
10. **エラー発生時のロールバック**: 在庫が不足しているなどの理由で例外が発生した場合、それまでのすべての処理はキャンセル（ロールバック）されます。
