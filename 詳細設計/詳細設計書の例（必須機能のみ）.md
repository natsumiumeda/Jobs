...
sequenceDiagram
    ...
    AdminUser->>FE: 商品管理一覧画面へアクセス / 特定商品の詳細確認
    FE->>ProductController: GET /api/admin/products (一覧取得) または GET /api/admin/products/{productId} (詳細取得)
    ProductController->>ProductService: getProducts(params) または getProductById(productId)

    ProductService->>ProductRepository: find(params) または findById(productId) // DBから商品情報を取得
    ProductRepository->>DB: SELECT item_id, item_name, stock_quantity, ... FROM items WHERE ...
    DB-->>ProductRepository: 商品情報リスト / 単一商品情報
    ProductRepository-->>ProductService: List<Product> / Optional<Product>
    ProductService->>ProductService: 商品情報からDTOへ変換 (stock_quantity含む)
    ProductService-->>ProductController: ProductList DTO / Product DTO

    ProductController-->>FE: 200 OK レスポンス (JSON: 商品データ、簡易在庫数を含む)
    FE-->>AdminUser: 商品管理一覧画面 / 商品詳細画面を表示 (各商品の簡易在庫数を表示)
</div>
...
| 商品詳細表示        | `GET`    | `/api/products/{productId}` |
| カート追加         | `POST`   | `/api/cart`                 |
| カート編集（数量変更など） | `PUT`    | `/api/cart/{itemId}`        |
| カート削除         | `DELETE` | `/api/cart/{itemId}`        |
| 注文情報入力（仮注文）   | `POST`   | `/api/orders/confirm`       |
| 注文確定          | `POST`   | `/api/orders/complete`      |
...
- 更新頻度の高いカラムにはインデックスを追加（INSERT/UPDATEのコスト増大）。
