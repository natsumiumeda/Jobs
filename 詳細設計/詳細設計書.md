...
|----|---------------|----------------|--------|-----------|---------------------------------|
| 1  | product_id    | CHAR(36)            | ○      | ○         | 商品ID（UUID）                   |
...
erDiagram
  ...
  PRODUCT {
    CHAR product_id PK "商品ID（UUIDと記載）"
    VARCHAR name "商品名"
    TEXT description "商品説明"
    INT price "商品価格（税抜）"
    INT stock "商品在庫数"
    VARCHAR material "素材"
    VARCHAR image_path "商品画像の保存パスまたはURL"
    DATETIME created_at "作成日時"
    DATETIME updated_at "最終更新日時"
  }

  ORDERS {
    CHAR order_id PK "注文ID（UUIDと記載）"
    DATETIME order_date "注文日"
  }

  ORDER_DETAIL {
    INT order_detail_id PK "注文明細ID"
    INT order_id FK "ORDERSテーブルの外部キー"
    INT product_id FK "PRODUCTテーブルの外部キー"
    VARCHAR product_name "商品名（注文時点）"
    INT price "商品単価（注文時点）"
    INT quantity "注文数"
  }

  PRODUCT ||--o{ ORDER_DETAIL : "参照される"
  ORDERS ||--o{ ORDER_DETAIL : "注文に含まれる"
  </div>
